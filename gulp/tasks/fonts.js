...
export const ttfToWoff = () => {
	...
	return (
		app.gulp
			...
			// Вигружаємо в папку з результатом
			.pipe(app.gulp.dest(`${app.path.build.fonts}`))
			// Шукаємо файли шрифтів .woff і woff2
...
export const fontsStyle = () => {
	...
	fs.readdir(app.path.build.fonts, function (err, fontsFiles) {
		if (fontsFiles) {
			// Перевіряємо чи існують файли стилів для підключення шритфів
			if (!fs.existsSync(fontsFile)) {
				// Якщо файлу немає, тоді створюємо його
				fs.writeFile(fontsFile, "", cb);
				let newFileOnly;
				for (var i = 0; i < fontsFiles.length; i++) {
					// Записуємо підключення шритфів в файл стилів
				...
				// Якщо файл є, виводимо сповіщення
				console.log(
